...
========================================================== */
if (!function_exists('dht_shop_mobile_image_candidate_is_loadable')) {
    function dht_shop_mobile_image_candidate_is_loadable($candidate) {
...
if (!function_exists('dht_shop_mobile_add_attachment_candidates')) {
    function dht_shop_mobile_add_attachment_candidates(&$candidates, $attachment_id, $size = 'woocommerce_thumbnail') {
...
if (!function_exists('dht_shop_mobile_get_media_candidates')) {
    function dht_shop_mobile_get_media_candidates($product_id, $size = 'woocommerce_thumbnail') {
        global $wpdb;
        ...
        if ($product && is_a($product, 'WC_Product')) {
            dht_shop_mobile_add_attachment_candidates($candidates, $product->get_image_id(), $size);
            foreach ((array) $product->get_gallery_image_ids() as $gallery_id) {
                dht_shop_mobile_add_attachment_candidates($candidates, $gallery_id, $size);
            }
        }
        ...
        if ($tables_available[$tables_key]) {
            ...
            foreach ((array) $rows as $row) {
                ...
                if ($attachment_id > 0) {
                    dht_shop_mobile_add_attachment_candidates($candidates, $attachment_id, $size);
                }
...
if (!function_exists('dht_shop_mobile_get_supplier_candidates')) {
    function dht_shop_mobile_get_supplier_candidates($product_id) {
        global $wpdb;
...
if (!function_exists('dht_shop_mobile_get_logo_candidate')) {
    function dht_shop_mobile_get_logo_candidate() {
...
if (!function_exists('dht_shop_mobile_get_product_candidates')) {
    function dht_shop_mobile_get_product_candidates($product_id, $size = 'woocommerce_thumbnail') {
        $all = array_merge(
            dht_shop_mobile_get_media_candidates($product_id, $size),
            dht_shop_mobile_get_supplier_candidates($product_id)
        );

        $logo = dht_shop_mobile_get_logo_candidate();
        if ($logo) {
            $all[] = $logo;
        }
...
if (!function_exists('dht_shop_mobile_get_best_product_image')) {
    function dht_shop_mobile_get_best_product_image($product_id, $size = 'woocommerce_thumbnail') {
        static $cache = array();
        ...
        foreach (dht_shop_mobile_get_product_candidates($product_id, $size) as $candidate) {
            if (dht_shop_mobile_image_candidate_is_loadable($candidate)) {
                $cache[$key] = $candidate;
                return $candidate;
            }
        }
...
if (!function_exists('dht_shop_mobile_product_image_filter')) {
    function dht_shop_mobile_product_image_filter($image, $product, $size, $attr, $placeholder, $original = '') {
        if (!is_a($product, 'WC_Product')) {
            return $image;
        }

        $selected = dht_shop_mobile_get_best_product_image($product->get_id(), $size);
        ...
        if (!$is_logo) {
            $logo = dht_shop_mobile_get_logo_candidate();
            if ($logo && !empty($logo['url']) && dht_shop_mobile_image_candidate_is_loadable($logo)) {
...
if (!function_exists('dht_shop_mobile_term_image_url')) {
    function dht_shop_mobile_term_image_url($term, $size = 'medium_large') {
...
if (!function_exists('dht_shop_mobile_term_link')) {
    function dht_shop_mobile_term_link($term) {
            ...
            <?php if (!empty($hero_categories)) : ?>
                <div class="sf-hero-picks shop-hero-picks" aria-label="Categorías destacadas">
                    <?php foreach ($hero_categories as $index => $term) :
                        $image = dht_shop_mobile_term_image_url($term, 'large');
                        ?>
                        <a class="sf-hero-pick sf-hero-pick--<?php echo esc_attr((string) ($index + 1)); ?>" href="<?php echo esc_url(dht_shop_mobile_term_link($term)); ?>">
                            <?php if ($image) : ?>
        ...
        <nav class="sf-quick-nav shop-quick-nav" aria-label="Categorías de producto">
            <div class="sf-shell sf-quick-nav-row">
                <?php foreach ($root_categories as $term) : ?>
                    <a href="<?php echo esc_url(dht_shop_mobile_term_link($term)); ?>"><?php echo esc_html($term->name); ?></a>
                <?php endforeach; ?>
            </div>
        </nav>
    <?php endif; ?>
            ...
            <div class="sf-shell">
                ...
                <div class="shop-category-grid">
                    <?php foreach ($category_cards as $term) :
                        $image = dht_shop_mobile_term_image_url($term, 'medium_large');
                        ?>
                        <a class="shop-category-card" href="<?php echo esc_url(dht_shop_mobile_term_link($term)); ?>">
        ...
        <div class="sf-shell">
            <div class="shop-catalog-panel">
                ...
                <?php if (function_exists('woocommerce_product_loop') && woocommerce_product_loop()) : ?>
                    ...
                    <?php add_filter('woocommerce_product_get_image', 'dht_shop_mobile_product_image_filter', 20, 6); ?>
                    ...
                    <?php remove_filter('woocommerce_product_get_image', 'dht_shop_mobile_product_image_filter', 20); ?>
