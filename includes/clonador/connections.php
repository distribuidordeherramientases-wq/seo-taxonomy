 ...
 *
 * Guarda las conexiones MySQL de PRO y STAGING usadas por el Clonador.
 * PRO se trata siempre como origen y puede usar credenciales de solo lectura.
 * STAGING puede requerir escritura cuando se ejecuta la clonacion integral
...
if ( ! function_exists( 'seo_clonador_db_allowed_envs' ) ) {
    function seo_clonador_db_allowed_envs() {
        return [
            'pro'     => 'Producción',
            'staging' => 'Staging',
        ];
...
if ( ! function_exists( 'seo_clonador_db_crypto_key' ) ) {
    function seo_clonador_db_crypto_key() {
        return hash( 'sha256', wp_salt( 'auth' ) . '|seo-system-environment-db|v1', true );
    }
}

if ( ! function_exists( 'seo_clonador_db_encrypt' ) ) {
    function seo_clonador_db_encrypt( $plain ) {
        $plain = (string) $plain;
        if ( '' === $plain ) {
            return '';
        }
        if ( ! function_exists( 'openssl_encrypt' ) || ! function_exists( 'random_bytes' ) ) {
            return new WP_Error(
                'seo_clonador_db_crypto_unavailable',
        ...
        try {
            ...
            return new WP_Error( 'seo_clonador_db_crypto_random', 'No se pudo generar un vector aleatorio para cifrar la contraseña.' );
        }
        ...
        if ( false === $ciphertext || 16 !== strlen( $tag ) ) {
            return new WP_Error( 'seo_clonador_db_crypto_failed', 'No se pudo cifrar la contraseña de la base de datos.' );
        }
...
if ( ! function_exists( 'seo_clonador_db_decrypt' ) ) {
    function seo_clonador_db_decrypt( $stored ) {
...
if ( ! function_exists( 'seo_clonador_db_option' ) ) {
    function seo_clonador_db_option() {
        $saved = get_option( 'seo_clonador_db_connections', [] );
        if ( is_array( $saved ) && ! empty( $saved ) ) {
            return $saved;
        }

        // Migracion unica desde la configuracion usada por el antiguo Comparador.
        $legacy = get_option( 'seo_environment_db_connections', [] );
        if ( is_array( $legacy ) && ! empty( $legacy ) ) {
            foreach ( [ 'pro', 'staging' ] as $env ) {
                if ( empty( $legacy[ $env ] ) || ! is_array( $legacy[ $env ] ) ) {
                    continue;
                }
                if ( ! empty( $legacy[ $env ]['disposable_mirror'] ) ) {
                    $legacy[ $env ]['destructive_clone'] = 1;
                    $legacy[ $env ]['clone_mode'] = 'destructive_clone';
                }
                unset( $legacy[ $env ]['disposable_mirror'], $legacy[ $env ]['staging_mode'] );
            }
            update_option( 'seo_clonador_db_connections', $legacy, false );
            return $legacy;
        }
...
if ( ! function_exists( 'seo_clonador_db_constant_name' ) ) {
    function seo_clonador_db_constant_name( $env, $field ) {
...
if ( ! function_exists( 'seo_clonador_db_constant_value' ) ) {
    function seo_clonador_db_constant_value( $env, $field, &$defined = false ) {
        $defined = false;
        $name    = seo_clonador_db_constant_name( $env, $field );
...
if ( ! function_exists( 'seo_clonador_db_settings' ) ) {
     ...
     */
    function seo_clonador_db_settings( $env ) {
        ...
        $saved_all = seo_clonador_db_option();
        ...
        foreach ( [ 'host', 'port', 'database', 'username', 'prefix', 'ssl_ca' ] as $field ) {
            $is_constant = false;
            $constant    = seo_clonador_db_constant_value( $env, $field, $is_constant );
            if ( $is_constant ) {
        ...
        $password           = seo_clonador_db_constant_value( $env, 'password', $password_constant );
        if ( $password_constant ) {
            $settings['password']      = (string) $password;
            $sources['password']       = 'constant';
        } else {
            $settings['password']      = seo_clonador_db_decrypt( (string) ( $settings['password_cipher'] ?? '' ) );
...
if ( ! function_exists( 'seo_clonador_db_is_configured' ) ) {
    function seo_clonador_db_is_configured( array $settings ) {
...
if ( ! function_exists( 'seo_clonador_db_open' ) ) {
    ...
    function seo_clonador_db_open( $env ) {
        ...
        if ( ! seo_clonador_db_is_configured( $settings ) ) {
...
if ( ! function_exists( 'seo_clonador_db_redirect_url' ) ) {
    function seo_clonador_db_redirect_url( array $args = [] ) {
...
if ( ! function_exists( 'seo_clonador_db_save_handler' ) ) {
    function seo_clonador_db_save_handler() {
        ...
        if ( 'staging' === $env ) {
            $settings['destructive_clone'] = ! empty( $_POST['destructive_clone'] ) ? 1 : 0;
            $settings['clone_mode'] = $settings['destructive_clone'] ? 'destructive_clone' : '';
        }

        if ( ! preg_match( '/^[A-Za-z0-9_]+$/', $settings['prefix'] ) ) {
            wp_safe_redirect( seo_clonador_db_redirect_url( [ 'env_db_status' => 'error', 'env_db_message' => 'El prefijo solo puede contener letras, numeros y guion bajo.' ] ) );
            exit;
        }
        ...
        if ( ! empty( $_POST['clear_password'] ) ) {
            ...
            if ( '' !== $password ) {
                $encrypted = seo_clonador_db_encrypt( $password );
                if ( is_wp_error( $encrypted ) ) {
                    wp_safe_redirect( seo_clonador_db_redirect_url( [ 'env_db_status' => 'error', 'env_db_message' => $encrypted->get_error_message() ] ) );
                    exit;
                }
        ...
        $all[ $env ] = $settings;
        update_option( 'seo_clonador_db_connections', $all, false );

        wp_safe_redirect( seo_clonador_db_redirect_url( [ 'env_db_saved' => $env ] ) );
        exit;
    }
}
add_action( 'admin_post_seo_clonador_db_save', 'seo_clonador_db_save_handler' );
...
add_action( 'admin_post_seo_clonador_db_test', 'seo_clonador_db_test_handler' );

if ( ! function_exists( 'seo_clonador_db_render_connections' ) ) {
    function seo_clonador_db_render_connections() {
        $allowed = seo_clonador_db_allowed_envs();

        echo '<section id="seo-clonador-db-connections" style="border:2px solid #8c8f94;border-radius:7px;padding:18px;margin:22px 0;background:#f6f7f7;">';
        echo '<div style="display:flex;justify-content:space-between;gap:16px;align-items:flex-start;flex-wrap:wrap;">';
        echo '<div><p style="margin:0 0 5px;font-size:12px;text-transform:uppercase;color:#646970;font-weight:600;">Clonador de catálogo</p><h3 style="margin:0 0 6px;">Conexiones BBDD PRO y STAGING</h3><p style="margin:0;max-width:980px;">PRO es el origen de solo lectura. STAGING es el único destino de escritura. El Clonador vacía el perímetro gestionado de STAGING y lo reconstruye desde PRO.</p></div>';
        echo '<span style="display:inline-block;padding:5px 9px;border-radius:12px;background:#fff8e5;">PRO → STAGING</span>';
        echo '</div>';
        ...
        foreach ( $allowed as $env => $label ) {
            $settings   = seo_clonador_db_settings( $env );
            $configured = seo_clonador_db_is_configured( $settings );
            ...
            echo '<input type="hidden" name="action" value="seo_clonador_db_save">';
            echo '<input type="hidden" name="env" value="' . esc_attr( $env ) . '">';
            wp_nonce_field( 'seo_clonador_db_save', 'seo_clonador_db_nonce' );
            ...
            if ( 'staging' === $env ) {
                echo '<p class="description" style="grid-column:1/-1;margin:0;"><strong>Clonar PRO → STAGING:</strong> esta conexión es el único destino de escritura y necesita SELECT, INSERT, UPDATE y DELETE sobre la BBDD de STAGING. PRO nunca se escribe.</p>';
                echo '<label style="grid-column:1/-1;padding:10px;border:1px solid #dba617;background:#fff8e5;border-radius:4px;"><input type="checkbox" name="destructive_clone" value="1" ' . checked( ! empty( $settings['destructive_clone'] ), true, false ) . '> <strong>STAGING_MODE = destructive_clone · STAGING es un laboratorio desechable para el perímetro del Clonador.</strong><br><span class="description">Al activar esta política autorizas que “Clonar PRO → STAGING” elimine de STAGING los objetos y asignaciones gestionados que no existan en PRO.</span></label>';
            } else {
            ...
            foreach ( $fields as $field => $def ) {
                ...
                if ( $readonly ) {
                    echo '<br><span class="description">Definido en <code>' . esc_html( seo_clonador_db_constant_name( $env, $field ) ) . '</code>.</span>';
                }
                echo '</label>';
            }
            ...
            if ( 'constant' === $password_source ) {
                echo '<div style="grid-column:1/-1;"><strong>Contraseña</strong><br><span style="display:inline-block;margin-top:5px;padding:4px 8px;border-radius:12px;background:#edfaef;">Definida en wp-config.php</span><br><span class="description"><code>' . esc_html( seo_clonador_db_constant_name( $env, 'password' ) ) . '</code></span></div>';
            } else {
            ...
            echo '<input type="hidden" name="action" value="seo_clonador_db_test">';
            echo '<input type="hidden" name="env" value="' . esc_attr( $env ) . '">';
            wp_nonce_field( 'seo_clonador_db_test', 'seo_clonador_db_test_nonce' );
...
if ( ! function_exists( 'seo_clonador_current_env' ) ) {
    function seo_clonador_current_env() {
