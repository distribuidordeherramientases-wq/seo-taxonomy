...
if ( ! defined( 'SEO_ENVIRONMENT_COMPARE_VERSION' ) ) {
    define( 'SEO_ENVIRONMENT_COMPARE_VERSION', '2.3.0' );
}
...
const ajax='.wp_json_encode( admin_url( 'admin-ajax.php' ) ).',nonce='.wp_json_encode( $nonce ).',scanOrder='.wp_json_encode( $scan_order ).',scanLabels='.wp_json_encode( $scan_labels ).',masterEntities='.wp_json_encode( array_fill_keys( array_filter( $scan_order, 'seo_environment_compare_is_master_entity' ), true ) ).';
...
function setBusy(on){uiBusy=!!on;document.querySelectorAll(".seo-env-scan,.seo-env-scan-all,.seo-env-align-all,.seo-env-bulk,.seo-env-sync-one").forEach(b=>{if(on){if(!b.hasAttribute("data-prebusy-disabled"))b.dataset.prebusyDisabled=b.disabled?"1":"0";b.disabled=true;}else{b.disabled=b.dataset.prebusyDisabled==="1";b.removeAttribute("data-prebusy-disabled");}});}
