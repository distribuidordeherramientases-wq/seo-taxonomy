...
final class SEO_Dependiente_Index {
    ...
    private static function get_seo_attributes($product_id) {
        ...
        foreach ((array) $rows as $row) {
            $type = trim((string) (is_object($row) ? ($row->attribute_type ?? '') : ($row['attribute_type'] ?? '')));
            $value = trim((string) (is_object($row) ? ($row->attribute_value ?? '') : ($row['attribute_value'] ?? '')));
            if ($type === '' || $value === '') {
                continue;
            }

            $name = trim((string) (is_object($row) ? ($row->attribute_name ?? '') : ($row['attribute_name'] ?? '')));
            $label = $name !== '' ? $name : ucwords(str_replace(array('_', '-'), ' ', $type));
            $scope = trim((string) (is_object($row) ? ($row->ambito ?? 'global') : ($row['ambito'] ?? 'global')));
            if ($scope === '') {
                $scope = 'global';
            }
