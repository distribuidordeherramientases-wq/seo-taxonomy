...
final class SEO_Clonador_Process {
    ...
    public static function init() {
        ...
        add_filter('seo_processes_monitor_items', array(__CLASS__, 'filter_monitor_items'), 20, 1);
        add_action('wp_loaded', array(__CLASS__, 'maybe_ensure_supervisor_worker'), 50);
    ...
    public static function maybe_ensure_supervisor_worker() {
        // Nunca inicia una clonación. Solo mantiene el controlador de un job que
        // el administrador ya dejó expresamente con run_requested=1.
        if (!self::is_running()) {
            return;
        }
        $result = self::ensure_supervisor_worker();
        if (!is_wp_error($result) && function_exists('seo_process_supervisor_nudge')) {
            seo_process_supervisor_nudge(0, 'clonador_keepalive');
        }
    }

    private static function ensure_supervisor_worker() {
        if (!function_exists('seo_process_supervisor_start')) {
            return true;
        }
        $result = seo_process_supervisor_start(false, 'clonador');
        if (is_wp_error($result)) {
            if (function_exists('seo_process_supervisor_log')) {
                seo_process_supervisor_log('error', 'clone_supervisor_start_failed', $result->get_error_message(), 'Clonador para Academia');
            }
            return $result;
        }
        return true;
    ...
    /**
     * Fila del monitor general de Procesos. Usa exactamente el mismo estado que
     * la pantalla del Clonador; no existe un segundo porcentaje ni un segundo
     * concepto de "completado".
     */
    public static function filter_monitor_items($items) {
        $items = is_array($items) ? array_values($items) : array();
        $row = self::monitor_item();
        $out = array();
        $inserted = false;
        foreach ($items as $item) {
            $id = sanitize_key((string) ($item['id'] ?? ''));
            $name = sanitize_text_field((string) ($item['name'] ?? ''));
            if (in_array($id, array('clonador', 'clonador-academia'), true) || in_array($name, array('Clonador', 'Clonador para Academia'), true)) {
                continue;
            }
            $out[] = $item;
            if ('classifier' === $id) {
    ...
    private static function monitor_item() {
        $state = self::state();
        ...
        $phase_no = absint($progress['phase_number'] ?? 0);
        $phase_total = absint($progress['phase_total'] ?? 0);
        $copied = absint($progress['copied_total'] ?? 0);
        $speed = self::normalize_speed($state['speed'] ?? 3);
        $profiles = self::speed_profiles();
        $profile = $profiles[$speed];

        $supervisor = function_exists('seo_process_supervisor_state') ? (array) seo_process_supervisor_state() : array();
        $supervisor_active = function_exists('seo_process_supervisor_is_active') ? (bool) seo_process_supervisor_is_active($supervisor) : false;
        $backend = sanitize_key((string) ($supervisor['backend'] ?? 'periodic_manager'));
        $backend_label = function_exists('seo_process_supervisor_backend_label')
            ? seo_process_supervisor_backend_label($backend)
            : ($backend ?: 'Gestor de procesos');
        $activity_ts = max(absint($state['heartbeat_at'] ?? 0), absint($supervisor['heartbeat_at'] ?? 0));
        $age = $activity_ts ? max(0, time() - $activity_ts) : null;

        if (function_exists('seo_processes_state')) {
            if ('completed' === $status) {
                $ui_state = seo_processes_state('completed', 'Parado · completado', 'completed');
            } elseif ('failed' === $status) {
                $ui_state = seo_processes_state('error', 'Error', 'error');
            } elseif ('paused' === $status) {
                $ui_state = seo_processes_state('stopped', 'Parado por usuario', 'stopped');
            } elseif ('running' === $status && $supervisor_active) {
                $ui_state = seo_processes_state('running', 'En ejecución', 'running');
            } elseif ('running' === $status && null !== $age && $age <= 90) {
                $ui_state = seo_processes_state('waiting', 'Esperando worker', 'waiting');
            } elseif ('running' === $status) {
                $ui_state = seo_processes_state('error', 'Sin worker activo', 'error');
            } elseif ('queued' === $status) {
                $ui_state = seo_processes_state('waiting', 'En cola', 'waiting');
            } else {
                $ui_state = seo_processes_state('stopped', 'Parado', 'stopped');
            }
        } else {
            $ui_state = array('code' => $status, 'label' => ucfirst($status), 'tone' => 'stopped');
        }

        $phase_label = sanitize_text_field((string) ($progress['phase_label'] ?? $state['phase'] ?? ''));
        $tables = array_values(array_filter(array_map('sanitize_text_field', (array) ($progress['current_tables'] ?? array()))));
        $load = 'Motor: ' . $backend_label . ' · velocidad ' . $speed . '/5 · ' . $profile['label'];
        if ($phase_no && $phase_total) {
            $load .= ' · fase ' . $phase_no . '/' . $phase_total;
        }
        $response = number_format_i18n($copied) . ' filas/objetos copiados';
        if (null !== $age) {
            $response .= ' · heartbeat ' . (function_exists('seo_processes_format_age') ? seo_processes_format_age($age) : ($age . ' s'));
        }
        $detail = sanitize_text_field((string) ($state['message'] ?? ''));
        if ($phase_label) {
            $detail = ($detail ? $detail . ' ' : '') . 'Fase: ' . $phase_label . '.';
        }
        if ($tables) {
            $detail .= ($detail ? ' ' : '') . 'Tabla(s): ' . implode(', ', $tables) . '.';
        }
        if (!$supervisor_active && 'running' === $status) {
            $detail .= ' El job sigue marcado como iniciado, pero no hay un worker persistente con heartbeat reciente.';
        }

        $url = function_exists('seo_processes_admin_url')
            ? seo_processes_admin_url('seo-import-export', array('seo_ie_tab' => 'clonador'))
            : admin_url('admin.php?page=seo-import-export&seo_ie_tab=clonador');

        return array(
            'id' => 'clonador',
            'name' => 'Clonador',
            'kind' => 'COPY PRO → STAGING · Gestor de workers',
            'state' => $ui_state,
            'speed' => 'Velocidad ' . $speed . '/5 · ' . $profile['label'],
            'response' => $response,
            'load' => $load,
            'activity' => function_exists('seo_processes_format_age') ? seo_processes_format_age($age) : (null === $age ? 'Sin actividad registrada' : $age . ' s'),
            'activity_age' => $age,
            'progress' => $percent,
            'progress_text' => number_format_i18n($percent) . '%',
            'detail' => $detail ?: 'Sin clonación activa.',
            'url' => $url,
            'can_start' => false,
            'start_label' => 'Reanudar',
        );
    }
    ...
    public static function start_new($preview, $speed = 3) {
        ...
        $controller = self::ensure_supervisor_worker();
