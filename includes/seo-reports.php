...
/*******************************************************************************
 * SISTEMA DE REPORTES SEO (ESTRUCTURA DE PESTAÑAS JERÁRQUICAS)
 ******************************************************************************/
...
function seo_reports_page() {
    ...
    $active_tab = isset($_GET['tab']) ? sanitize_key($_GET['tab']) : 'report';
    $base_url = admin_url('admin.php?page=seo-reports');
    
    echo '<div class="wrap">';
    echo '<h1>SEO Reports (Sistema Jerárquico)</h1>';
    
    // Pestaña renombrada aquí de 'Estructura Total' a 'Categorías'[cite: 1]
    echo '<h2 class="nav-tab-wrapper">';
    echo '<a class="nav-tab ' . ($active_tab === 'report' ? 'nav-tab-active' : '') . '" href="' . esc_url($base_url . '&tab=report') . '">Report</a>';
    echo '<a class="nav-tab ' . ($active_tab === 'dashboard' ? 'nav-tab-active' : '') . '" href="' . esc_url($base_url . '&tab=dashboard') . '">Dashboard</a>';
    ...
    echo '<a class="nav-tab ' . ($active_tab === 'google_intelligence' ? 'nav-tab-active' : '') . '" href="' . esc_url($base_url . '&tab=google_intelligence') . '">Google Intelligence</a>';
    echo '<a class="nav-tab ' . ($active_tab === 'post_opportunities' ? 'nav-tab-active' : '') . '" href="' . esc_url($base_url . '&tab=post_opportunities') . '">Posts · Oportunidades</a>';
    echo '</h2>';
    ...
    if ($active_tab === 'dashboard') {
        ...
        seo_render_hierarchy_report(false);
    }
    echo '</div>';
}
...
/**
 * PESTAÑAS: Report y Report Detallado
 */
function seo_render_hierarchy_report($show_excerpt = false) {
    global $wpdb;
    ...
    echo '<h2>📊 Jerarquía del sistema</h2>';
