<?php
/**
 * SEO System - Marketing, sitemaps, auditoria publica y estilo visual.
 *
 ...
 */
$seo_google_service = dirname(__DIR__) . '/import-export/suppliers/google-search.php';
if (is_readable($seo_google_service)) {
    require_once $seo_google_service;
}
unset($seo_google_service);

$seo_landing_module = dirname(__DIR__) . '/seo-landing-pages.php';
if (is_readable($seo_landing_module)) {
    require_once $seo_landing_module;
}
unset($seo_landing_module);

$seo_social_network_module = dirname(__DIR__) . '/social-network/social-bootstrap.php';
...
function seo_menu_manager_marketing_page()
{
    ...
    if ('scan' === $requested_tab) {
        wp_safe_redirect(add_query_arg(array('page'=>'seo-menu-marketing','tab'=>'sitemaps'), admin_url('admin.php')));
        exit;
    }
...
function seo_marketing_render_sitemaps_tab($notice)
{
    ...
    if (function_exists('seo_marketing_scan_ensure_tables')) {
        seo_marketing_scan_ensure_tables();
    }
    $audit_by_sitemap = function_exists('seo_marketing_scan_stats_by_sitemap')
        ? seo_marketing_scan_stats_by_sitemap()
        : array();
    ...
    echo '<thead><tr><th>Archivo</th><th>XML</th><th>URLs</th><th>Auditadas</th><th>OK</th><th>Avisos</th><th>Errores</th><th>Cobertura</th><th>Tamaño</th><th>Modificado</th></tr></thead><tbody>';

    foreach ($report['files'] as $file) {
        $is_index = $file['filename'] === 'sitemap.xml';
        $audit = !$is_index && isset($audit_by_sitemap[$file['url']])
            ? $audit_by_sitemap[$file['url']]
            : array('total'=>0,'checked'=>0,'ok'=>0,'warnings'=>0,'errors'=>0);
        $coverage = !$is_index && (int) $file['count'] > 0
            ? min(100, round(((int) $audit['checked'] / (int) $file['count']) * 100, 1))
            : 0;
        ...
        if ($is_index) {
            ...
            echo '<td>' . esc_html(number_format_i18n((int) $audit['checked'])) . '</td>';
            echo '<td><span style="color:#1d6b43;font-weight:700;">' . esc_html(number_format_i18n((int) $audit['ok'])) . '</span></td>';
            echo '<td>' . ((int) $audit['warnings'] > 0 ? '<span style="color:#8a5a00;font-weight:700;">' . esc_html(number_format_i18n((int) $audit['warnings'])) . '</span>' : '0') . '</td>';
            echo '<td>' . ((int) $audit['errors'] > 0 ? '<span style="color:#c62828;font-weight:700;">' . esc_html(number_format_i18n((int) $audit['errors'])) . '</span>' : '0') . '</td>';
        ...
        if (!$file['valid'] && $file['error'] !== '') {
            echo '<tr><td colspan="10"><span style="color:#c62828;">' . esc_html($file['error']) . '</span></td></tr>';
        }
    }
    ...
    if (function_exists('seo_marketing_render_scan_tab')) {
...
function seo_marketing_scan_admin_url($args = array())
{
    return add_query_arg(
        array_merge(
            array('page' => 'seo-menu-marketing', 'tab' => 'sitemaps'),
            is_array($args) ? $args : array()
        ),
        admin_url('admin.php')
    );
 ...
 */
function seo_marketing_scan_runner_control()
{
    $defaults = array(
        'remote_batch'        => 100,
        'load_batch'          => 60,
    ...
    $stored = function_exists('seo_processes_control_for')
        ? seo_processes_control_for('health-sitemap')
        : array();
    $control = wp_parse_args(is_array($stored) ? $stored : array(), $defaults);

    $control['remote_batch'] = max(10, min(SEO_MARKETING_SCAN_BATCH_LIMIT, absint($control['remote_batch'])));
    $control['load_batch'] = max(5, min(2000, absint($control['load_batch'])));
    $control['max_workers'] = max(1, min(16, absint($control['max_workers'])));
    $control['initial_workers'] = max(1, min($control['max_workers'], absint($control['initial_workers'])));
    $control['fast_p95_ms'] = max(100, min(10000, absint($control['fast_p95_ms'])));
    $control['slow_p95_ms'] = max($control['fast_p95_ms'] + 100, min(20000, absint($control['slow_p95_ms'])));
    $control['very_slow_p95_ms'] = max($control['slow_p95_ms'] + 100, min(60000, absint($control['very_slow_p95_ms'])));
    $control['min_interval_ms'] = max(10, min(5000, absint($control['min_interval_ms'])));
    $control['initial_interval_ms'] = max($control['min_interval_ms'], min(10000, absint($control['initial_interval_ms'])));
    $control['max_interval_ms'] = max($control['initial_interval_ms'], min(30000, absint($control['max_interval_ms'])));
...
function seo_marketing_scan_get_runner_config()
{
    ...
    return array(
        ...
        'control'           => seo_marketing_scan_runner_control(),
    );
}
...
function seo_marketing_scan_inventory_stats()
{
    ...
    $row = $wpdb->get_row(
        "SELECT
            ...
            SUM(active_in_sitemap=1 AND status_bucket='seo_warning') AS seo_warning_count,
            SUM(active_in_sitemap=1 AND status_bucket='seo_error') AS seo_error_count,
    ...
    $defaults = array(
        ...
        'seo_warning_count' => 0, 'seo_error_count' => 0,
...
/**
 * Devuelve el tipo lógico de recurso a partir del sitemap que lo contiene.
 */
function seo_marketing_scan_resource_type_from_sitemap($sitemap_url)
{
    $name = strtolower((string) basename((string) wp_parse_url($sitemap_url, PHP_URL_PATH)));
    if (strpos($name, 'sitemap-products-') === 0) return 'product';
    if (strpos($name, 'sitemap-product-categories-') === 0) return 'category';
    if (strpos($name, 'sitemap-posts-') === 0) return 'post';
    if (strpos($name, 'sitemap-pages-') === 0) return 'page';
    if (strpos($name, 'sitemap-clusters-') === 0) return 'cluster';
    if (strpos($name, 'sitemap-hubs-') === 0) return 'hub';
    return 'url';
}

/**
 * Copia una instantánea del inventario activo a la cola de una ejecución.
 * El modo full encola todo lo publicado; load_test solo una muestra limitada.
 * Nada de esto se envía de golpe a GitHub: el worker lo irá pidiendo por lotes.
 */
function seo_marketing_scan_enqueue_inventory($scan_id, $mode = 'full', $limit = 0)
{
    global $wpdb;
    $tables = seo_marketing_scan_tables();
    ...
    $mode = $mode === 'load_test' ? 'load_test' : 'full';
    $remaining = $mode === 'load_test' ? max(1, absint($limit)) : PHP_INT_MAX;
    $cursor = 0;
    $inserted_total = 0;
    $read_chunk = 500;

    while ($remaining > 0) {
        $take = min($read_chunk, $remaining);
        $rows = $wpdb->get_results(
            $wpdb->prepare(
                "SELECT id,url,sitemap_url FROM {$tables['inventory']}
                 WHERE active_in_sitemap=1 AND id>%d
        ...
        $params = array();
        foreach ($rows as $row) {
            $inventory_id = absint($row['id']);
            $cursor = max($cursor, $inventory_id);
            $url = (string) $row['url'];
            $sitemap_url = (string) $row['sitemap_url'];
            $values[] = "(%d,%d,%s,%s,'queued',%s,%s)";
            $params[] = $scan_id;
            $params[] = $inventory_id;
            $params[] = md5($url);
            $params[] = seo_marketing_scan_resource_type_from_sitemap($sitemap_url);
            $params[] = $sitemap_url;
            $params[] = $url;
        }
...
/**
 * KPIs del estado persistente agrupados por sitemap hijo actual.
 */
function seo_marketing_scan_stats_by_sitemap()
{
    global $wpdb;
    $tables = seo_marketing_scan_tables();
    $rows = $wpdb->get_results(
        "SELECT sitemap_url,
            ...
            SUM(status_bucket='ok') AS ok_count,
            SUM(status_bucket IN ('3xx','seo_warning')) AS warnings,
            SUM(status_bucket IN ('404','403','4xx','5xx','network','seo_error')) AS errors
         FROM {$tables['inventory']}
         WHERE active_in_sitemap=1
         GROUP BY sitemap_url",
        ARRAY_A
    );
    $out = array();
    foreach ((array) $rows as $row) {
        $key = (string) ($row['sitemap_url'] ?? '');
        if ($key === '') continue;
        $out[$key] = array(
...
/**
 * Crea una ejecución completa y dispara GitHub una sola vez.
 * WordPress guarda el snapshot completo en SQL; GitHub nunca recibe todo a la vez.
 */
function seo_marketing_scan_handle_start()
{
    ...
    // El inventario siempre se reconstruye a partir del sitemap.xml vigente y sus hijos dinámicos.
    ...
    $mode = isset($_POST['scan_mode']) ? sanitize_key(wp_unslash($_POST['scan_mode'])) : 'full';
    $mode = $mode === 'load_test' ? 'load_test' : 'full';
    $control = isset($config['control']) && is_array($config['control']) ? $config['control'] : seo_marketing_scan_runner_control();
    $queue_limit = $mode === 'load_test' ? absint($control['load_batch'] ?? 60) : 0;
    ...
    $inserted = $wpdb->insert(
        ...
        array(
            ...
            'selection_mode'      => $mode,
            'batch_limit'         => absint($control['remote_batch'] ?? 100),
    ...
    $queued = seo_marketing_scan_enqueue_inventory($scan_id, $mode, $queue_limit);
    if (is_wp_error($queued) || (int) $queued <= 0) {
        $message = is_wp_error($queued) ? $queued->get_error_message() : 'No se encontraron URLs para encolar.';
        $wpdb->update(
            $tables['scans'],
    ...
    $wpdb->update(
        $tables['scans'],
        array('total_urls' => (int) $queued, 'updated_at' => $now),
        array('id' => $scan_id),
        array('%d','%s'),
        array('%d')
    );
...
/**
 * GitHub pide aquí el siguiente lote pequeño de la ejecución completa.
 * No se marca como consumido hasta recibir el callback, de modo que un GET perdido
 * puede repetirse sin saltarse URLs. El callback es idempotente.
 */
function seo_marketing_scan_batch_endpoint(WP_REST_Request $request)
{
    ...
    $tables = seo_marketing_scan_tables();
    $control = seo_marketing_scan_runner_control();
    $limit = max(10, min(SEO_MARKETING_SCAN_BATCH_LIMIT, absint($control['remote_batch'] ?? 100)));

    $items = $wpdb->get_results(
        $wpdb->prepare(
            "SELECT url,sitemap_url,resource_type FROM {$tables['urls']}
             WHERE scan_id=%d AND queue_status='queued'
             ORDER BY id ASC LIMIT %d",
            (int) $scan['id'],
            $limit
        ),
    ...
    $remaining = (int) $wpdb->get_var(
        $wpdb->prepare(
            "SELECT COUNT(*) FROM {$tables['urls']} WHERE scan_id=%d AND queue_status='queued'",
            (int) $scan['id']
        )
    );
...
function seo_marketing_scan_result_bucket($row)
{
    ...
    $error_type = sanitize_key((string) ($row['error_type'] ?? ''));

    // Señales SEO ligeras del <head>. No convierten el barrido en una auditoría profunda.
    if ($error_type === 'canonical_mismatch') return 'seo_warning';
    if (in_array($error_type, array('noindex','non_html'), true)) return 'seo_error';

    if (in_array($final, array(404,410), true)) return '404';
    if ($final === 403) return '403';
    if ($final >= 500 && $final <= 599) return '5xx';
    if ($final >= 400 && $final <= 499) return '4xx';
    if ($redirects > 0) return '3xx';
    if ($final === 200 && $error_type === '') return 'ok';
    return 'network';
}
...
function seo_marketing_scan_callback(WP_REST_Request $request)
{
    ...
    if ($event === 'complete') {
        ...
        $remaining = (int) $wpdb->get_var(
            $wpdb->prepare("SELECT COUNT(*) FROM {$tables['urls']} WHERE scan_id=%d AND queue_status='queued'", $scan_id)
        );
        if ($remaining > 0) {
            return new WP_Error('seo_scan_incomplete', 'El worker intento completar la ejecucion con URLs pendientes.', array('status' => 409, 'pending' => $remaining));
        }
        $wpdb->update(
            $tables['scans'],
            array(
...
/**
 * Expone el escaneo de sitemap al monitor central de Procesos sin acoplar
 * Marketing a la implementación interna del monitor.
 */
function seo_marketing_scan_process_monitor_item($items)
{
    if (!is_array($items)) {
        $items = array();
    }
    ...
    seo_marketing_scan_ensure_tables();
    $active = seo_marketing_scan_active_run();
    $latest = seo_marketing_scan_latest_run();
    $run = is_array($active) ? $active : (is_array($latest) ? $latest : array());
    $control = seo_marketing_scan_runner_control();

    $status = (string) ($run['status'] ?? '');
    if (is_array($active)) {
        $state = function_exists('seo_processes_state')
            ? seo_processes_state('running', 'En ejecución', 'running')
            : array('code'=>'running','label'=>'En ejecución','tone'=>'running');
    } elseif ($status === 'completed') {
        $state = function_exists('seo_processes_state')
            ? seo_processes_state('completed', 'Parado · completado', 'completed')
            : array('code'=>'completed','label'=>'Parado · completado','tone'=>'completed');
    } elseif (in_array($status, array('failed','cancelled'), true)) {
        $state = function_exists('seo_processes_state')
            ? seo_processes_state('error', $status === 'failed' ? 'Error' : 'Cancelado', 'error')
            : array('code'=>'error','label'=>$status === 'failed' ? 'Error' : 'Cancelado','tone'=>'error');
    } else {
        $state = function_exists('seo_processes_state')
            ? seo_processes_state('stopped', 'Parado', 'stopped')
            : array('code'=>'stopped','label'=>'Parado','tone'=>'stopped');
    }

    $processed = absint($run['processed_urls'] ?? 0);
    $total = absint($run['total_urls'] ?? 0);
    $progress = $total > 0 ? max(0, min(100, (int) round(($processed / $total) * 100))) : null;

    $elapsed = !empty($run['duration_ms']) ? max(0.001, ((float) $run['duration_ms']) / 1000) : 0.0;
    if ($elapsed <= 0 && !empty($run['started_at'])) {
        $started = strtotime((string) $run['started_at'] . ' UTC');
        if ($started) {
            $elapsed = max(0.001, time() - $started);
        }
    }
    $rate = ($elapsed > 0 && $processed > 0) ? (($processed / $elapsed) * 60) : 0;

    $activity = 'Sin actividad registrada';
    if (!empty($run['updated_at'])) {
        $updated = strtotime((string) $run['updated_at'] . ' UTC');
        if ($updated && function_exists('seo_processes_format_age')) {
            $activity = seo_processes_format_age(max(0, time() - $updated));
        }
    }

    $detail = empty($run)
        ? 'Todavía no hay ejecuciones registradas para el sitemap público.'
        : (($run['selection_mode'] ?? '') === 'load_test' ? 'Test de carga' : 'Escaneo completo') . ': ' . number_format_i18n($processed) . '/' . number_format_i18n($total) . ' URLs.';
    ...
    $items[] = array(
        'id'            => 'health-sitemap',
        'name'          => 'Chequeo · Sitemap público',
        'kind'          => 'Worker GitHub',
        'state'         => $state,
        'speed'         => function_exists('seo_processes_format_rate') ? seo_processes_format_rate($rate, 'peticiones') : ($rate > 0 ? round($rate, 1) . ' peticiones/min' : 'Sin ritmo medible'),
        'response'      => 'Barrido HTTP ligero',
        'load'          => 'lote remoto ' . number_format_i18n(absint($control['remote_batch'])) . ' · objetivo ' . number_format_i18n(absint($control['initial_workers'])) . '–' . number_format_i18n(absint($control['max_workers'])) . ' workers',
        'activity'      => $activity,
        'activity_age'  => null,
        'progress'      => $progress,
        'progress_text' => null !== $progress ? number_format_i18n($progress) . '%' : '—',
        'detail'        => $detail,
        'url'           => seo_marketing_scan_admin_url(),
    );

    return $items;
}
add_filter('seo_processes_monitor_items', 'seo_marketing_scan_process_monitor_item');
...
function seo_marketing_scan_bucket_label($bucket)
{
    $labels = array(
        ...
        'ok' => 'OK',
        '3xx' => 'Redireccion',
        'seo_warning' => 'Canonical',
        'seo_error' => 'No indexable',
        '404' => '404/410',
        '403' => '403',
        '4xx' => 'Otros 4xx',
        '5xx' => '5xx',
...
function seo_marketing_scan_render_notice()
{
    ...
    if ($message === 'inventory_synced') {
        $text = sprintf(
            'Inventario actualizado desde el sitemap actual: %s URLs activas. %s URLs han salido del sitemap.',
            number_format_i18n(absint($_GET['total'] ?? 0)),
            number_format_i18n(absint($_GET['removed'] ?? 0))
        );
    } elseif ($message === 'queued') {
        $mode = sanitize_key((string) ($_GET['mode'] ?? 'full'));
        $text = sprintf(
            '%s enviado a GitHub: %s URLs encoladas localmente. El worker las pedirá poco a poco.',
            $mode === 'load_test' ? 'Test de carga' : 'Escaneo completo',
            number_format_i18n(absint($_GET['batch'] ?? 0))
        );
    } elseif ($message === 'busy') {
        $class = 'notice notice-warning';
        $text = 'Ya hay un escaneo en curso. Puedes detenerlo antes de lanzar otro.';
    } elseif ($message === 'cancelled') {
        $text = 'Proceso detenido. Ya puedes iniciar otro escaneo.';
    } elseif ($message === 'cancel_not_running') {
        $class = 'notice notice-warning';
        $text = 'Ese proceso ya no estaba en ejecución.';
    } elseif ($message === 'cancel_missing') {
        $class = 'notice notice-error';
        $text = 'No se pudo identificar el proceso que querías detener.';
    } elseif ($message === 'empty') {
        $class = 'notice notice-warning';
        $text = 'El sitemap actual no contiene URLs activas disponibles para escanear.';
    } elseif ($message === 'config') {
        $class = 'notice notice-error';
        $text = 'La conexión GitHub Python Runner está incompleta.';
        ...
        $class = 'notice notice-error';
        $text = 'No se pudo preparar la cola del escaneo en la base de datos.';
    }
...
function seo_marketing_scan_inventory_filter_sql($filter)
{
    switch ($filter) {
        ...
        case 'seo_warning': return "active_in_sitemap=1 AND status_bucket='seo_warning'";
        case 'seo_error': return "active_in_sitemap=1 AND status_bucket='seo_error'";
...
function seo_marketing_scan_render_inventory_table($stats)
{
    global $wpdb;
    $tables = seo_marketing_scan_tables();
    $filter = isset($_GET['inventory_filter']) ? sanitize_key(wp_unslash($_GET['inventory_filter'])) : 'issues';
    $allowed = array('all','ok','issues','pending','3xx','seo_warning','seo_error','404','403','4xx','5xx','network','removed');
    ...
    $list_sql = "SELECT * FROM {$tables['inventory']} WHERE {$where}
        ...
        CASE status_bucket WHEN '5xx' THEN 1 WHEN 'network' THEN 2 WHEN '404' THEN 3 WHEN '403' THEN 4 WHEN '4xx' THEN 5 WHEN 'seo_error' THEN 6 WHEN '3xx' THEN 7 WHEN 'seo_warning' THEN 8 WHEN 'pending' THEN 9 ELSE 10 END,
    ...
    $filters = array(
        ...
        'seo_warning' => array('Canonical', $stats['seo_warning_count']),
        'seo_error' => array('Noindex / no HTML', $stats['seo_error_count']),
    ...
    echo '<div class="seo-scan-card"><h2 style="margin-top:0">Log de URLs e incidencias</h2>';
    ...
    echo '<input type="hidden" name="page" value="seo-menu-marketing"><input type="hidden" name="tab" value="sitemaps"><input type="hidden" name="inventory_filter" value="' . esc_attr($filter) . '">';
    ...
    if (empty($rows)) {
        ...
        foreach ($rows as $row) {
            ...
            $badge_class = !$is_active || $bucket === 'pending' ? 'info' : ($bucket === 'ok' ? 'ok' : (in_array($bucket, array('3xx','seo_warning'), true) ? 'warn' : 'err'));
            $http = '—';
...
function seo_marketing_scan_render_latest_run($scan)
{
    ...
    echo '<div class="seo-scan-card"><h2 style="margin-top:0">Ultimo escaneo</h2>';
    echo '<p><span class="seo-scan-badge ' . esc_attr($badge) . '">' . esc_html(seo_marketing_scan_status_label($status)) . '</span> &nbsp; <strong>Modo:</strong> ' . esc_html($scan['selection_mode'] === 'load_test' ? 'Test de carga' : 'Completo') . '</p>';
    ...
    seo_marketing_scan_render_kpi((int) $scan['other_errors'], 'SEO / otros', (int) $scan['other_errors'] ? 'warn' : 'ok');
    echo '</div>';
...
function seo_marketing_render_scan_tab()
{
    ...
    $warnings = (int) $stats['s3xx'] + (int) $stats['seo_warning_count'];
    $errors = max(0, (int) $stats['issues'] - $warnings);

    echo '<div class="seo-scan-card">';
    echo '<h2 style="margin-top:0">Chequeo público de todo lo publicado en sitemaps</h2>';
    echo '<p>La fuente de verdad es siempre el <code>sitemap.xml</code> generado actualmente. El inventario descubre sus hijos de forma dinámica; no existe una lista fija de archivos. Un clic crea una ejecución completa y el worker pide lotes pequeños sucesivos hasta terminar. Por URL se revisan HTTP, redirecciones, SSL/red, <code>noindex</code>, tipo de contenido y canonical cuando está presente.</p>';
    ...
    echo '<form method="post" action="' . esc_url(admin_url('admin-post.php')) . '"><input type="hidden" name="action" value="seo_marketing_scan_start"><input type="hidden" name="scan_mode" value="full">';
    wp_nonce_field('seo_marketing_scan_start');
    $start_disabled = !empty($missing) || !empty($active_scan);
    submit_button('Escanear sitemap completo', 'primary', 'submit', false, $start_disabled ? array('disabled'=>'disabled') : array());
    echo '</form>';

    echo '<form method="post" action="' . esc_url(admin_url('admin-post.php')) . '"><input type="hidden" name="action" value="seo_marketing_scan_start"><input type="hidden" name="scan_mode" value="load_test">';
    wp_nonce_field('seo_marketing_scan_start');
    submit_button('Test de carga seguro', 'secondary', 'submit', false, $start_disabled ? array('disabled'=>'disabled') : array());
    echo '</form>';

    if (!empty($active_scan)) {
        echo '<form method="post" action="' . esc_url(admin_url('admin-post.php')) . '" onsubmit="return confirm(\'¿Detener este proceso de escaneo? Los resultados ya recibidos se conservan.\');">';
    ...
    if (!empty($active_scan)) {
        $run_coverage = (int) $active_scan['total_urls'] > 0
            ? min(100, round(((int) $active_scan['processed_urls'] / (int) $active_scan['total_urls']) * 100, 1))
            : 0;
        echo '<p><strong>Escaneo activo:</strong> ' . esc_html(seo_marketing_scan_status_label((string) $active_scan['status'])) . ' · ' . esc_html(number_format_i18n((int) $active_scan['processed_urls'])) . '/' . esc_html(number_format_i18n((int) $active_scan['total_urls'])) . ' (' . esc_html(number_format_i18n($run_coverage, 1)) . '%).</p>';
    }

    echo '<div class="seo-scan-kpis">';
    seo_marketing_scan_render_kpi($stats['total'], 'URLs publicadas', '');
    seo_marketing_scan_render_kpi($stats['checked'], 'Comprobadas', '');
    seo_marketing_scan_render_kpi($stats['pending'], 'Nunca comprobadas', $stats['pending'] ? 'warn' : 'ok');
    seo_marketing_scan_render_kpi($stats['ok_count'], 'Correctas', 'ok');
    seo_marketing_scan_render_kpi($warnings, 'Avisos / redirecciones / canonical', $warnings ? 'warn' : 'ok');
    seo_marketing_scan_render_kpi($errors, 'Errores', $errors ? 'err' : 'ok');
    echo '</div>';

    echo '<h3>Cobertura persistente: ' . esc_html(number_format_i18n($coverage, 1)) . '%</h3>';
    echo '<div class="seo-scan-progress"><span style="width:' . esc_attr((string) $coverage) . '%"></span></div>';
    echo '<p class="description">La cobertura persistente indica qué URLs del sitemap actual han sido comprobadas al menos una vez. El progreso del escaneo activo se muestra por separado.</p>';
    echo '</div>';
...
function seo_marketing_scan_render_config($config, $missing)
{
    ...
    echo '<tr><th>Lote remoto</th><td>' . esc_html(number_format_i18n((int) $control['remote_batch'])) . ' URLs por petición del worker</td></tr>';
    echo '<tr><th>Workers</th><td>' . esc_html(number_format_i18n((int) $control['initial_workers'])) . ' iniciales · máximo ' . esc_html(number_format_i18n((int) $control['max_workers'])) . '</td></tr>';
    echo '<tr><th>Conexión</th><td>' . (empty($missing) ? '<span class="seo-scan-badge ok">Operativa</span>' : '<span class="seo-scan-badge err">Incompleta</span>') . '</td></tr>';
    echo '</tbody></table>';
    echo '<p class="description">WordPress mantiene el snapshot completo en SQL. GitHub solicita solo el siguiente lote y repite hasta que no quedan URLs. La velocidad se toma de <strong>Procesos → Gestor de workers → Chequeo de sitemap</strong>.</p>';
    if (!empty($missing)) {
...
name: SEO Taxonomy - Sitemap continuous audit

on:
  workflow_dispatch:
    inputs:
      scan_id:
        description: "ID unico del escaneo"
        required: true
        type: string
      batch_url:
        description: "Endpoint WordPress que entrega el siguiente lote dinamico"
        required: true
        type: string
      callback_url:
...
jobs:
  audit:
    ...
    steps:
      ...
      - name: Audit complete sitemap queue
        env:
...
#!/usr/bin/env python3
"""Auditor continuo y adaptativo del sitemap de SEO Taxonomy.

WordPress conserva el snapshot completo del sitemap en SQL. Este proceso nunca
recibe todo el inventario: solicita un lote pequeno, lo procesa, devuelve sus
resultados y pide el siguiente hasta que no queda ninguno.
"""
...
USER_AGENT = "SEO-System-Sitemap-Auditor/3.0 (+continuous-dynamic-sitemap-health-check)"
...
PRESSURE_ERROR_TYPES = {"timeout", "connection_error"}
...
class AdaptiveLoadController:
    def __init__(self) -> None:
        ...
        self.initial_workers = 2
        self.max_workers = 8
        self.target_workers = 2
        self.fast_p95_ms = 800
        self.slow_p95_ms = 1500
        self.very_slow_p95_ms = 2500
        self.min_interval_ms = 120
        self.interval_ms = 300
        self.max_interval_ms = 3000

    def apply_control(self, control: dict) -> None:
        if not isinstance(control, dict):
            return
        with self._lock:
            self.max_workers = clamp(control.get("max_workers", self.max_workers), 1, 16)
            requested_initial = clamp(control.get("initial_workers", self.initial_workers), 1, self.max_workers)
            self.initial_workers = requested_initial
            self.target_workers = clamp(self.target_workers, 1, self.max_workers)
            if self.target_workers < requested_initial:
                self.target_workers = requested_initial
            self.fast_p95_ms = clamp(control.get("fast_p95_ms", self.fast_p95_ms), 100, 10000)
            self.slow_p95_ms = max(self.fast_p95_ms + 100, clamp(control.get("slow_p95_ms", self.slow_p95_ms), 200, 20000))
            self.very_slow_p95_ms = max(self.slow_p95_ms + 100, clamp(control.get("very_slow_p95_ms", self.very_slow_p95_ms), 300, 60000))
            self.min_interval_ms = clamp(control.get("min_interval_ms", self.min_interval_ms), 10, 5000)
            initial_interval = clamp(control.get("initial_interval_ms", self.interval_ms), self.min_interval_ms, 10000)
            self.max_interval_ms = max(initial_interval, clamp(control.get("max_interval_ms", self.max_interval_ms), 10, 30000))
            self.interval_ms = clamp(self.interval_ms, self.min_interval_ms, self.max_interval_ms)
            if self.interval_ms < initial_interval and self.target_workers <= requested_initial:
                self.interval_ms = initial_interval

    def wait_before_request(self) -> None:
        with self._lock:
            interval = self.interval_ms / 1000.0
        spacing = max(0.01, interval * random.uniform(0.90, 1.10))
        now = time.monotonic()
    ...
    def max_pool_workers(self) -> int:
        with self._lock:
            return self.max_workers

    def note_wave(self, results: List[dict]) -> None:
        if not results:
            return
        times = [int(r.get("response_ms") or 0) for r in results if int(r.get("response_ms") or 0) > 0]
        p95 = 0
        if times:
            ordered = sorted(times)
            p95 = ordered[max(0, min(len(ordered)-1, int(len(ordered)*0.95 + 0.999999)-1))]
        pressure = any(
            int(r.get("final_status") or r.get("http_status") or 0) in PRESSURE_STATUSES
            or str(r.get("error_type") or "") in PRESSURE_ERROR_TYPES
            for r in results
        )
        with self._lock:
            if pressure or (p95 and p95 >= self.very_slow_p95_ms):
                self.target_workers = max(1, self.target_workers // 2)
                self.interval_ms = min(self.max_interval_ms, max(self.interval_ms + 100, int(self.interval_ms * 1.6)))
            elif p95 and p95 >= self.slow_p95_ms:
                self.target_workers = max(1, self.target_workers - 1)
                self.interval_ms = min(self.max_interval_ms, max(self.interval_ms + 50, int(self.interval_ms * 1.25)))
            elif p95 and p95 <= self.fast_p95_ms:
                self.target_workers = min(self.max_workers, self.target_workers + 1)
                self.interval_ms = max(self.min_interval_ms, int(self.interval_ms * 0.90))

    def note_api_pressure(self) -> None:
        with self._lock:
            self.target_workers = max(1, self.target_workers // 2)
            self.interval_ms = min(self.max_interval_ms, max(self.interval_ms + 200, int(self.interval_ms * 1.8)))

    def status_text(self) -> str:
        with self._lock:
            return f"workers={self.target_workers}/{self.max_workers} intervalo={self.interval_ms}ms"
...
class HeadSignalParser(HTMLParser):
    """Extrae solo señales SEO ligeras del <head>."""
    def __init__(self) -> None:
...
def normalized_compare_url(url: str) -> str:
    try:
...
def read_head_signals(response: Response, max_bytes: int = 262144) -> Tuple[bool, str]:
    ...
    for chunk in response.iter_content(chunk_size=16384):
        ...
        chunks.append(piece)
        consumed += len(piece)
        text_probe = b"".join(chunks[-3:]).lower()
        if b"</head" in text_probe or consumed >= max_bytes:
            break
    raw = b"".join(chunks)
    ...
    parser.feed(text)
    return parser.noindex, parser.canonical


def response_chain(response: Response) -> List[dict]:
    chain: List[dict] = []
    for hop in response.history:
        chain.append({"status": int(hop.status_code), "url": hop.url, "location": hop.headers.get("Location", "")})
    if response.history:
...
def classify_response(initial: int, final: int, redirects: int) -> Tuple[str, str]:
    if redirects > 0:
        if final in (404, 410): return "redirect_to_404", f"Redireccion termina en HTTP {final}"
        if final == 403: return "redirect_to_403", "Redireccion termina en HTTP 403"
        if final == 429: return "redirect_to_429", "Redireccion termina en HTTP 429"
        if 500 <= final <= 599: return "redirect_to_5xx", f"Redireccion termina en HTTP {final}"
        if final == 200: return "redirect", f"La URL del sitemap redirige ({initial} -> 200)"
        return "redirect_error", f"Redireccion termina en HTTP {final}"
    if final == 200: return "", ""
    if final in (404, 410): return ("http_404" if final == 404 else "http_410"), f"HTTP {final}"
    if final == 403: return "http_403", "HTTP 403"
    if final == 429: return "rate_limited", "HTTP 429"
    if 500 <= final <= 599: return "http_5xx", f"HTTP {final}"
    if 400 <= final <= 499: return "http_4xx", f"HTTP {final}"
    if 300 <= final <= 399: return "http_3xx", f"HTTP {final} sin destino final"
    return "http_status", f"HTTP inesperado {final}"


def check_url(item: dict, timeout: float, controller: AdaptiveLoadController) -> dict:
    url = str(item.get("url") or "").strip()
    sitemap_url = str(item.get("sitemap_url") or "").strip()
    resource_type = str(item.get("resource_type") or "url").strip()
    ...
    try:
        ...
        try:
            ...
            if final == 200:
                x_robots = str(response.headers.get("X-Robots-Tag", "")).lower()
                content_type = str(response.headers.get("Content-Type", "")).lower()
                if "noindex" in x_robots:
                    error_type, error_message = "noindex", "X-Robots-Tag contiene noindex"
                elif "text/html" in content_type or "application/xhtml+xml" in content_type or not content_type:
                    noindex, canonical = read_head_signals(response)
                    if noindex:
                        error_type, error_message = "noindex", "Meta robots/googlebot/bingbot contiene noindex"
                    elif canonical:
                        canonical_url = urljoin(response.url, canonical)
                        if normalized_compare_url(canonical_url) != normalized_compare_url(response.url):
                            error_type, error_message = "canonical_mismatch", f"Canonical apunta a {canonical_url}"
                else:
                    error_type, error_message = "non_html", f"Content-Type inesperado: {content_type or 'desconocido'}"
...
def api_request(method: str, url: str, token: str, controller: AdaptiveLoadController, json_payload: Optional[dict] = None) -> Response:
    ...
    for attempt in range(len(BACKOFF_SECONDS) + 1):
        try:
            response = requests.request(method, url, headers=headers, json=json_payload, timeout=30.0)
            if 200 <= response.status_code < 300:
                return response
            last_error = RuntimeError(f"HTTP {response.status_code}: {response.text[:500]}")
            transient = response.status_code in TRANSIENT_API_STATUSES
            response.close()
            if not transient:
                raise last_error
        ...
        if attempt < len(BACKOFF_SECONDS):
            time.sleep(BACKOFF_SECONDS[attempt] * random.uniform(0.9, 1.1))
    raise RuntimeError(f"No se pudo completar la llamada API: {last_error}")


def fetch_batch(batch_url: str, token: str, controller: AdaptiveLoadController) -> dict:
    response = api_request("GET", batch_url, token, controller)
    try:
        payload = response.json()
    finally:
        response.close()
    if not isinstance(payload, dict) or not isinstance(payload.get("items"), list):
        raise RuntimeError("El endpoint de lote no devolvio un JSON valido")
    if len(payload["items"]) > MAX_BATCH_URLS:
        raise RuntimeError(f"El lote contiene {len(payload['items'])} URLs; maximo {MAX_BATCH_URLS}")
    return payload


def send_callback(callback_url: str, token: str, payload: dict, controller: AdaptiveLoadController) -> None:
    response = api_request("POST", callback_url, token, controller, json_payload=payload)
    response.close()
...
def bucket(result: dict) -> str:
    ...
    if final in (404, 410): return "404"
    if final == 403: return "403"
    if 500 <= final <= 599: return "5xx"
    if 400 <= final <= 499: return "other"
    if redirects > 0: return "3xx"
    if final == 200 and not result.get("error_type"): return "200"
    return "other"


def process_remote_batch(items: List[dict], timeout: float, controller: AdaptiveLoadController) -> List[dict]:
    pending = list(items)
    results: List[dict] = []
    while pending:
        workers = min(controller.current_workers(), len(pending))
        wave = pending[:workers]
        del pending[:workers]
        with concurrent.futures.ThreadPoolExecutor(max_workers=controller.max_pool_workers()) as executor:
            futures = [executor.submit(check_url, item, timeout, controller) for item in wave]
            wave_results = [future.result() for future in futures]
        results.extend(wave_results)
        controller.note_wave(wave_results)
    return results


def run(args: argparse.Namespace) -> int:
    token = os.getenv("SEO_SCAN_CALLBACK_TOKEN", "").strip()
    if not token:
    ...
    try:
        send_callback(args.callback_url, token, {"scan_id": args.scan_id, "event": "start"}, controller)

        while True:
            payload = fetch_batch(args.batch_url, token, controller)
            controller.apply_control(payload.get("control") or {})
            declared_total = max(declared_total, int(payload.get("total") or 0))
            items = [item for item in payload.get("items", []) if isinstance(item, dict) and str(item.get("url") or "").startswith(("http://", "https://"))]
            if not items:
                break

            signature = tuple(str(item.get("url") or "") for item in items)
            if signature == previous_signature:
                raise RuntimeError("WordPress ha devuelto dos veces el mismo lote despues del callback; se detiene para evitar un bucle")
            previous_signature = signature

            results = process_remote_batch(items, args.timeout, controller)
            for result in results:
                counts[bucket(result)] += 1

            for offset in range(0, len(results), CALLBACK_BATCH):
                block = results[offset:offset + CALLBACK_BATCH]
                send_callback(
                    args.callback_url,
                    token,
                    {"scan_id": args.scan_id, "event": "batch", "results": block},
                    controller,
                )

            processed_total += len(results)
            print(
                f"Procesadas {processed_total}/{declared_total or '?'} URLs; "
                f"restantes servidor={payload.get('remaining', '?')} | {controller.status_text()}",
                flush=True,
            )

        summary = {
            "processed_urls": processed_total,
            "total_urls": declared_total,
        ...
        send_callback(args.callback_url, token, {"scan_id": args.scan_id, "event": "complete", "summary": summary}, controller)
        ...
        try:
            send_callback(args.callback_url, token, {"scan_id": args.scan_id, "event": "failed", "error_message": message[:2000]}, controller)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Audita de forma continua todo el snapshot actual del sitemap")
