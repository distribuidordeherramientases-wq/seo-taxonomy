...
if (!function_exists('seo_classifier_classify_product_labels')) {
    function seo_classifier_classify_product_labels($product_id, $current = null, array $options = []) {
        ...
        // Los jobs pueden limitar el cálculo a los huecos realmente solicitados.
        $requested_groups = [];
        if (!empty($options['groups']) && is_array($options['groups'])) {
            foreach ($options['groups'] as $requested_group) {
                $requested_group = sanitize_key((string)$requested_group);
                if (in_array($requested_group, ['tipo','rol','aplicacion','plataforma','subtipo'], true)) {
                    $requested_groups[] = $requested_group;
                }
            }
            $requested_groups = array_values(array_unique($requested_groups));
        }
        $ordered_groups = ['tipo','aplicacion','subtipo','plataforma'];
        if ($requested_groups) {
            $ordered_groups = array_values(array_filter($ordered_groups, static function($group) use ($requested_groups) {
                return in_array($group, $requested_groups, true);
            }));
        }
        $effective_type_ids = [];
        ...
        foreach ($ordered_groups as $group) {
            ...
            $new_rows = [];
            if (empty($current[$group]) && !$safe_rows && function_exists('seo_classifier_discover_new_label_terms')) {
                $new_rows = seo_classifier_discover_new_label_terms($group, $context, $current, $ranked, $options);
                if ($new_rows && !$review_rows) $state = 'new';
            }
            ...
            $groups[$group] = [
                ...
                'new'=>$new_rows,
                'confidence'=>$top ? round((float)($top['score'] ?? 0), 2) : ($new_rows ? (float)($new_rows[0]['score'] ?? 0.0) : 0.0),
                'candidates'=>$candidate_rows,
            ];

            if ($new_rows) $new_terms[$group] = $new_rows;
            if ($safe_labels) {
        ...
        // Si un job no ha solicitado ROL, no hacemos trabajo adicional ni lo añadimos al resultado.
        $should_resolve_role = !$requested_groups || in_array('rol', $requested_groups, true);
        if ($should_resolve_role) {
        ...
        if ($should_resolve_role && empty($current['rol'])) {
            $type_id = 0;
